...
func (g *BtcKeyGenerator) GenerateKeyPair() (address, publicKey, privateKey string, err error) {
	...
	// 转换为btcec私钥
	privKey, _ := btcec.PrivKeyFromBytes(btcec.S256(), crypto.FromECDSA(privateKeyECDSA))
...
func (g *BtcKeyGenerator) DeriveKeyPairFromPrivateKey(privateKey string) (address, publicKey string, err error) {
	...
	// 转换为btcec私钥
	privKey, err := btcec.PrivKeyFromBytes(btcec.S256(), privateKeyBytes)
	if err != nil {
		return "", "", fmt.Errorf("failed to convert to btcec private key: %w", err)
	}
