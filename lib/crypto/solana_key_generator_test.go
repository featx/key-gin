package crypto

import (
	"testing"

	"github.com/stretchr/testify/assert"
)

// SolanaKeyGenerator 测试用例
func TestSolanaKeyGenerator_GenerateKeyPair(t *testing.T) {
	generator := &SolanaKeyGenerator{}

	// 生成密钥对
	address, publicKey, privateKey, err := generator.GenerateKeyPair()

	// 验证结果
	assert.NoError(t, err)
	assert.NotEmpty(t, address)
	assert.NotEmpty(t, publicKey)
	assert.NotEmpty(t, privateKey)
	// 验证地址格式符合Solana规范
	assert.Contains(t, address, "1")
	// 验证私钥长度
	assert.Equal(t, 128, len(privateKey)) // 64字节的十六进制表示
}

func TestSolanaKeyGenerator_DeriveKeyPairFromPrivateKey(t *testing.T) {
	generator := &SolanaKeyGenerator{}

	// 先生成一个有效的私钥用于测试
	_, _, privateKey, err := generator.GenerateKeyPair()
	assert.NoError(t, err)

	// 从私钥派生公钥和地址
	address, publicKey, err := generator.DeriveKeyPairFromPrivateKey(privateKey)

	// 验证结果
	assert.NoError(t, err)
	assert.NotEmpty(t, address)
	assert.NotEmpty(t, publicKey)
	assert.Contains(t, address, "1")
}

func TestSolanaKeyGenerator_InvalidPrivateKey(t *testing.T) {
	generator := &SolanaKeyGenerator{}

	// 测试无效长度的私钥
	invalidPrivateKey := "00000000000000000000000000000001"
	address, publicKey, err := generator.DeriveKeyPairFromPrivateKey(invalidPrivateKey)

	// 验证错误处理
	assert.Error(t, err)
	assert.Empty(t, address)
	assert.Empty(t, publicKey)
	assert.Contains(t, err.Error(), "invalid private key length")

	// 测试非十六进制格式的私钥
	nonHexPrivateKey := "not_a_hex_string"
	address, publicKey, err = generator.DeriveKeyPairFromPrivateKey(nonHexPrivateKey)

	// 验证错误处理
	assert.Error(t, err)
	assert.Empty(t, address)
	assert.Empty(t, publicKey)
}

func TestSolanaKeyGenerator_PublicKeyToAddress(t *testing.T) {
	generator := &SolanaKeyGenerator{}

	// 先生成一个密钥对获取有效的公钥
	_, publicKey, _, err := generator.GenerateKeyPair()
	assert.NoError(t, err)

	// 从公钥生成地址
	address, err := generator.PublicKeyToAddress(publicKey)

	// 验证结果
	assert.NoError(t, err)
	assert.NotEmpty(t, address)
	assert.Contains(t, address, "1")
}